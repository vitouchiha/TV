...
git remote get-url upstream || git remote add upstream https://github.com/nzo66/TV.git
...
cp .env "$backup_dir/.env" 2>/dev/null || echo "⚠️ Nessun file .env da copiare"
...
cp "$backup_dir/.env" .env 2>/dev/null || echo "⚠️ Nessun .env da ripristinare"
